...
mapview(camaras_unique, cex=1, col.regions = "black") + 
  mapview(fotografias,  cex = "Fotos yaguarundi",
...
Mazama <- camaras %>% 
  ...
  dplyr::filter (Species =="Herpailurus yagouaroundi" )  %>%  
  as.data.frame() %>% 
  dlply(.(anfitrion)) # return list by anfitrion


# for (i in 1:2){ #length(Mazama)){
  # if(dim(Mazama[[i]])[1] <= 10) # skip if menos 10 records
   # next
 cat("###", names(Mazama)[2])
  # par(mar = c(5, 4, 3, 3) + 0.1)
  activityDensity (recordTable = Mazama[[2]],
...
# Foto
 ...
 [![yagouaroundi](https://dlizcano.github.io/activity/fig/yagouaroundi_small.jpg)]( https://dlizcano.github.io/activity/fig/yagouaroundi_large.jpg)
