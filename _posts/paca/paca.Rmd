---
title: "Activity Pattern for guagua"
description: |
  Activity Pattern for _Cuniculus paca_
author:
...
preview: https://dlizcano.github.io/activity/fig/paca.png
twitter:
  site: "@rstudio"
  creator: "@dlizcano"
---
...
La guagua (_Cuniculus paca_) es un mamífero común y abundante de tamaño mediano a grande. Conocido como guanta, paca o boruga.

### Detalles de Wikipedia

https://es.wikipedia.org/wiki/Cuniculus_paca
...
paca <- filter(camaras, Species=="Cuniculus paca")# & subproject_name=="El Escondite")
by_sp <- camaras %>%  group_by(Species) %>% tally()
fotografias <- paca %>%  group_by(deployment_id) %>% tally()
names(fotografias) <-  c("Camara", "Fotos guagua", "geometry")


mapview(camaras_unique, cex=1, col.regions = "black") + 
  mapview(fotografias,  cex = "Fotos guagua",
...
Cuniculus <- camaras %>% 
  ...
  dplyr::filter (Species =="Cuniculus paca" )  %>%  
  as.data.frame() %>% 
  dlply(.(anfitrion)) # return list by anfitrion


 for (i in 1:length(Cuniculus)){
  if(dim(Cuniculus[[i]])[1] <= 10) # skip if menos 10 records
    next
  cat("###", names(Cuniculus)[i])
  #plot.new( )
  #plot.window( xlim=c(-1,25), ylim=c(0,1))
  # par(mar = c(5, 4, 3, 3) + 0.1)
  activityDensity (recordTable = Cuniculus[[i]],
...
# Foto
 ...
 [![Boruga](https://dlizcano.github.io/activity/fig/Paca_small.jpg)]( https://dlizcano.github.io/activity/fig/Paca_large.jpg)
