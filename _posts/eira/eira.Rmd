...
tamandua_selva <- filter(camaras, Species=="Tamandua tetradactyla" & subproject_name==c("Selva Vides", "El Escondite"))
by_sp <- camaras %>%  group_by(Species) %>% tally()
fotografias <- tamandua_selva %>%  group_by(deployment_id) %>% tally()
names(fotografias) <-  c("Camara", "Fotos tamandua", "geometry")


library(terra); library(maptiles)
sierraFebpts <- st_as_sf(tamandua_selva, coords = c("longitude", "latitude"), crs=4326)
...
tamandua_window <- qtm(putumayo_osm1) + 
...
#  tm_polygons("Rio", colorNA =NULL, border.col = "blue", palette = "blue") +  #tm_symbols (size = 0.5) +
 tm_shape(fotografias) + # tm_symbols (col="red", size = 0.25) + 
    tm_bubbles(size = "Fotos tamandua", col = "red", border.col= "red", alpha= 0.5, 
...
mapview(camaras_unique, cex=1, col.regions = "black") + mapview(fotografias,  cex = "Fotos tamandua", alpha = 0, map.types=c("Esri.WorldImagery", "OpenTopoMap"))
...
Tamandua <- camaras %>% 
  ...
  dplyr::filter (Species =="Tamandua tetradactyla" )  %>% as.data.frame()

# par(mar = c(5, 4, 3, 3) + 0.1)
activityDensity (recordTable = Tamandua,
...
# Foto
 ...
 ![tamandua](https://dlizcano.github.io/activity/fig/tamandua_small.JPG)
 https://dlizcano.github.io/activity/fig/tamandua_large.JPG
