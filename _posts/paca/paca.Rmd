---
title: "Activity Pattern for paca"
description: |
...
Mamífero común y abundante de tamaño mediano a grande. Conocido como guanta, paca, boruga
...
paca_selva <- filter(camaras, Species=="Cuniculus paca" & subproject_name=="Selva Vides")
by_sp <- camaras %>%  group_by(Species) %>% tally()
by_sp_predio <- paca_selva %>%  group_by(deployment_id) %>% tally()
names(by_sp_predio) <-  c("Camara", "Fotos paca", "geometry")
...
#  tm_polygons("Rio", colorNA =NULL, border.col = "blue", palette = "blue") +  #tm_symbols (size = 0.5) +
 tm_shape(by_sp_predio) + # tm_symbols (col="red", size = 0.25) + 
    tm_bubbles(size = "Fotos paca", col = "red", border.col= "red", alpha= 0.5, 
...
cuniculus <- camaras %>% 
  ...
  dplyr::filter (Species =="Cuniculus paca" )  %>% as.data.frame()
...
# Foto
 ...
 ![Boruga](https://dlizcano.github.io/activity/fig/Paca_small.jpg)
 https://dlizcano.github.io/activity/fig/Paca_large.jpg
