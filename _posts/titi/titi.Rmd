---
title: "Activity Pattern for pichico monkey"
description: |
  Activity Pattern for _Saguinus fuscicollis_
author:
...
preview: https://dlizcano.github.io/activity/fig/saguinus.jpg
twitter:
  site: "@rstudio"
  creator: "@dlizcano"
---
...
### _Saguinus fuscicollis_

```{r eval=TRUE}



sp_place <- filter(camaras, Species=="Leontocebus fuscicollis" )#& subproject_name==c("Selva Vides", "El Escondite"))
by_sp <- camaras %>%  group_by(Species) %>% tally()
fotografias <- sp_place %>%  group_by(deployment_id) %>% tally()
names(fotografias) <-  c("Camara", "Fotos bebeleche", "geometry")
...
mapview(camaras_unique, cex=1, col.regions = "black") + mapview(fotografias,  cex = "Fotos bebeleche", alpha = 0, map.types=c("Esri.WorldImagery", "OpenTopoMap"))
...
eira <- camaras %>% 
  ...
  dplyr::filter (Species =="Leontocebus fuscicollis" )  %>% as.data.frame()
...
# Foto
 ...
 [![monkey](https://dlizcano.github.io/activity/fig/saguinus_small.jpg)](
 https://dlizcano.github.io/activity/fig/saguinus_large.jpg)
