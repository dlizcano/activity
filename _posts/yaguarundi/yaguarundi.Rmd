...
description: |
  Activity Pattern for _Herpailurus yagouaroundi_
author:
...
yagouaroundi_selva <- filter(camaras, Species=="Herpailurus yagouaroundi" & subproject_name=="El Escondite")
by_sp <- camaras %>%  group_by(Species) %>% tally()
fotografias <- yagouaroundi_selva %>%  group_by(deployment_id) %>% tally()
names(fotografias) <-  c("Camara", "Fotos yaguarundi", "geometry")
...
mapview(camaras_unique, cex=1, col.regions = "black") + mapview(fotografias,  cex = "Fotos yaguarundi", alpha = 0, map.types=c("Esri.WorldImagery", "OpenTopoMap"))
...
Mazama <- camaras %>% 
  ...
  dplyr::filter (Species =="Herpailurus yagouaroundi" )  %>% as.data.frame()
...
# Foto
 ...
 ![yagouaroundi](https://dlizcano.github.io/activity/fig/yagouaroundi_small.jpg)
 https://dlizcano.github.io/activity/fig/yagouaroundi_large.jpg
