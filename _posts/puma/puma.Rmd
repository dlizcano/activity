---
title: "Activity Pattern for Puma"
description: |
  Activity Pattern for _Puma concolor_
author:
...
preview: https://dlizcano.github.io/activity/fig/puma.png
twitter:
  site: "@rstudio"
  creator: "@dlizcano"
---
...
Mamífero poco común y poco abundante de tamaño grande. Conocido puma, tigre colorado
...
paca_selva <- filter(camaras, Species=="Puma concolor" & subproject_name=="Selva Vides")
by_sp <- camaras %>%  group_by(Species) %>% tally()
by_sp_predio <- paca_selva %>%  group_by(deployment_id) %>% tally()
names(by_sp_predio) <-  c("Camara", "Fotos puma", "geometry")
...
puma <- camaras %>% 
  ...
  dplyr::filter (Species =="Puma concolor" )  %>% as.data.frame()

# par(mar = c(5, 4, 3, 3) + 0.1)
activityDensity (recordTable = puma,
...
No hay suficientes regisros para hacer la grafica. 

# Foto

 ![Mazama](https://dlizcano.github.io/activity/fig/puma_small.jpg)
 https://dlizcano.github.io/activity/fig/puma_large.jpg
