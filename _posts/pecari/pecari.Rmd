---
title: "Activity Pattern for pecari"
description: |
...
paca_selva <- filter(camaras, Species=="Pecari tajacu" & subproject_name=="Selva Vides")
by_sp <- camaras %>%  group_by(Species) %>% tally()
by_sp_predio <- paca_selva %>%  group_by(deployment_id) %>% tally()
names(by_sp_predio) <-  c("Camara", "Fotos pecari", "geometry")
...
pecari <- camaras %>% 
  ...
  dplyr::filter (Species =="Pecari tajacu" )  %>% as.data.frame()

# par(mar = c(5, 4, 3, 3) + 0.1)
activityDensity (recordTable = pecari,
...
# Foto
 ...
 ![Pecari](https://dlizcano.github.io/activity/fig/pecari_small.jpg)
 https://dlizcano.github.io/activity/fig/pecari_large.jpg
